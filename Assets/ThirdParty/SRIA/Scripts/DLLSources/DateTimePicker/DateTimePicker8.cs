...
namespace frame8.Logic.Misc.Visual.UI.DateTimePicker
{
	...
	/// or drag and drop the prefab in /SRIA/Resources/SRIA/DateTimePicker8 into your scene and subscribe to <see cref="OnDateSelected"/>
	/// </summary>
	public class DateTimePicker8 : MonoBehaviour
	{
		...
		bool _AutoInit;
		[SerializeField]
		bool _DisplaySelectedDateAsShort;
		[SerializeField]
		bool _DisplaySelectedTimeAsShort;
		...
		const float SCROLL_DURATION1 = .5f;
		const float SCROLL_DURATION2 = .75f;
		const float SCROLL_DURATION3 = 1f;
		const float INIT_STEP_DURATION = .5f;
		const float ANIM_DURATION = .25f;
		const float DEFAULT_WIDTH = 660f, DEFAULT_HEIGHT = DEFAULT_WIDTH / 2;

		float AnimElapsedTime01 { get { float t = Mathf.Clamp01((Time.time - _AnimStartTime) / ANIM_DURATION); return t * t * t * t; } }
		...
		public static DateTimePicker8 Show(Action<DateTime> onSelected) { return Show(DateTime.Now, onSelected); }
		public static DateTimePicker8 Show(DateTime startingDate, Action<DateTime> onSelected) { return Show(startingDate, onSelected, DEFAULT_WIDTH, DEFAULT_HEIGHT); }
		public static DateTimePicker8 Show(DateTime startingDate, Action<DateTime> onSelected, float width, float height)
		{
			var go = Resources.Load<GameObject>("SRIA/DateTimePicker8");
			var picker = (Instantiate(go) as GameObject).GetComponent<DateTimePicker8>();
