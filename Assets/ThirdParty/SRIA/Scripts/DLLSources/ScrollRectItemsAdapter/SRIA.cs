...
namespace frame8.Logic.Misc.Visual.UI.ScrollRectItemsAdapter
{
	...
	/// <para>5. call MyScrollRectItemsAdapter.ResetItems(int) once (and any time your dataset is changed) and the following things will happen:</para>
	...
	where TItemViewsHolder : BaseItemViewsHolder
	{
		...
		/// <summary>Same as ResetItems(&lt;currentCount&gt;)</summary>
		...
		/// <returns>if no smooth scroll animation was already playing or <paramref name="overrideCurrentScrollingAnimation"/> is true</returns>
		...
		/// <para>It uses a different approach when content size is smaller than viewport's size, so it can yield consistent results for <see cref="SRIA{TParams, TItemViewsHolder}.ComputeVisibility(double)"/></para>
		/// </summary>
