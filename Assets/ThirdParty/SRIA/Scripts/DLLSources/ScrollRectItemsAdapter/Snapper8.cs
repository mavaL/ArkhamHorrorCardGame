...
namespace frame8.Logic.Misc.Visual.UI.ScrollRectItemsAdapter
{
	...
	public class Snapper8 : MonoBehaviour
	{
		...
		internal void StartSnappingIfNeeded()
		{
			...
			if (snapToNextOnly)				
			{
				bool loopingEnabled = _Adapter.BaseParameters.loopItems;
			...
			_Adapter.SmoothScrollTo(
				...
				progress =>
				{
					...
					bool done = progress == 1f;
					if (done || _SnappingCancelled || IsPointerDraggingOnScrollRect || IsPointerDraggingOnScrollbar) // done. last iteration
					{
		...
		AbstractViewsHolder GetMiddleVH(out float distanceToTarget) { return _Adapter.GetViewsHolderOfClosestItemToViewportPoint(viewportSnapPivot01, itemSnapPivot01, out distanceToTarget); }
