...
namespace frame8.Logic.Misc.Visual.UI.ScrollRectItemsAdapter
{
	...
	where TItemViewsHolder : BaseItemViewsHolder
	{
		...
		bool VirtualizeContentPositionIfNeeded(ref PointerEventData pev, double delta, bool returnPointerEventDataIfNeeded, bool alsoCorrectTransversalPositions)
		{
			...
			//Debug.Log(potentialResetDir + ", " + firstVisibleIsFirstIndexInView + ", " + lastVisibleIsLastIndexInView);
			if (potentialResetDir == 1)
			{
				// Commented to fix issue with snapper not functioning well when there are few items and they cover most of the viewport
				// TODO investigate if this optimization was actually needed in terms of FPS gain. Same with the analogue case below when potentialResetDir = 2
				...
				else
				{
					...
					if ((!_Params.loopItems || !lastVisibleIsLastIndexInView) && curRealAbstrNormPos >= 1d - _InternalState.ProximityToLimitNeeded01ToResetPos)
			...
			{
				// Commented to fix issue with snapper not functioning well when there are few items and they cover most of the viewport
				// TODO investigate if this optimization was actually needed in terms of FPS gain. Same with the analogue case above when potentialResetDir = 1
				//// Already done (this can sometimes mean there are too few items in list)
				...
				else
				{
					...
					if ((!_Params.loopItems || !firstVisibleIsFirstIndexInView) && curRealAbstrNormPos <= _InternalState.ProximityToLimitNeeded01ToResetPos)
